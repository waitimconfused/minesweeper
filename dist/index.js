...
function tick() {
    ...
    if (GameMap.isPlaying == false) {
        let targetZoom = Math.min((window.innerHeight - 100) / (GameMap.height * GameMap.scale), (window.innerWidth - 100) / (GameMap.width * GameMap.scale));
    ...
    if (GameMap.canvas)
        context.drawImage(GameMap.canvas, 0, 0);
